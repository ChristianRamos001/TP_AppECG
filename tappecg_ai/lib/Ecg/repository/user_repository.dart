...
class UserRepository{
  ...
  Future<void>saveToken(token, email, name, id) async{
  ...
  Future<String> loginRequest(String email, String password) async {
    ...
    if(response.statusCode == 200){
      ...
      UserHelper.id = extractData["idusuario"].toString();
      UserHelper.name = extractData["nombre"];
      saveToken(extractData["token"],email,UserHelper.name,extractData["idusuario"].toString());
